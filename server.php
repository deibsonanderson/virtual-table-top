...
if(isset($_POST)){	
	switch($_POST["type"]){
		case('get'):
		    $query = mysqli_query($conexao, "SELECT * FROM token WHERE sessao = '".$_POST["session"]."' ");
		    $html = '';
		    ...
		    $query = mysqli_query($conexao, "SELECT * FROM `sessao` WHERE `codigo` = '".$_POST['session']."' ");
		...
		case('post'):
			...
			mysqli_query($conexao, "UPDATE `sessao` SET `data_atualizacao` = NOW()  WHERE `codigo` = '".$_POST['session']."' ");
		break;
		...
		case('remove'):
		    mysqli_query($conexao, "DELETE FROM `token` WHERE `codigo` = '".$_POST['code']."'");
		    mysqli_query($conexao, "UPDATE `sessao` SET `data_atualizacao` = NOW()  WHERE `codigo` = '".$_POST['session']."' ");
		break;
		
		case('update_map'):
		    mysqli_query($conexao, "UPDATE `sessao` SET `mapa` = '".$_POST['map']."', `data_atualizacao` = NOW()  WHERE `codigo` = '".$_POST['session']."' ");
	    break;
	    
		case('zoom_map'):
		    mysqli_query($conexao, "UPDATE `sessao` SET `zoom` = '".$_POST['zoom']."', `data_atualizacao` = NOW()  WHERE `codigo` = '".$_POST['session']."' ");
	    break;
		...
		case('synchronize'):
			...
			while (time() - $startTime < $timeout) {
				$query = mysqli_query($conexao, "SELECT data_atualizacao FROM sessao WHERE codigo = '".$_POST['session']."' ");
				$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
